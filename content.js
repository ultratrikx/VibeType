class WebPilotController {
  ...
  addEventListeners() {
    ...
    window.addEventListener('message', (event) => {
      if (event.source !== this.sidebarIframe.contentWindow || event.data.type !== 'FROM_WEBPILOT_SIDEBAR') {
        return;
      }
  ...
  toggleSidebar() {
    ...
    if (isVisible) {
        ...
        this.postMessageToSidebar('updateCollapseState', { isCollapsed: false });
    }
  }

  toggleCollapse() {
    this.sidebarIframe.classList.toggle('is-collapsed');
    const isCollapsed = this.sidebarIframe.classList.contains('is-collapsed');
  ...
  async handleSidebarAction(action, data) {
    switch (action) {
      ...
      case 'getTabList':
        ...
        break;
      case 'loadTabContext':
        const contextResponse = await this.sendMessageToBackground({ action: 'getTabContext', tabId: parseInt(data.tabId) });
        if (contextResponse.success) {
          this.additionalContext = contextResponse.context;
          this.postMessageToSidebar('updateTabContext', { context: this.additionalContext });
        }
        break;
      case 'clearTabContext':
        this.additionalContext = null;
        this.postMessageToSidebar('updateTabContext', { context: null });
        break;
      case 'sendChatMessage':
        ...
        if (chatResponse.success) {
          this.postMessageToSidebar('addChatMessage', { sender: 'assistant', message: chatResponse.reply });
        } else {
          this.postMessageToSidebar('addChatMessage', { sender: 'assistant', message: `Error: ${chatResponse.error}` });
        }
        break;
      case 'loadSettings':
        const settings = await this.sendMessageToBackground({ action: 'getSettings' });
        this.postMessageToSidebar('settingsLoaded', { settings });
        break;
      case 'saveSetting':
        await this.sendMessageToBackground({ action: 'saveSetting', key: data.key, value: data.value });
        break;
      case 'testApiKey':
        const testResult = await this.sendMessageToBackground({ action: 'testApiKey' });
        this.postMessageToSidebar('apiKeyStatus', { success: testResult.success, message: testResult.message });
        break;
  ...
  async handleTextAction(action) {
    ...
    this.postMessageToSidebar('showLoading', { message: 'Analyzing text...' });
    ...
    if (response.success) {
      const suggestedText = response.improvedText || response.rewrittenText || response.elaboratedText;
      this.postMessageToSidebar('showConfirmation', { originalText, suggestedText });
    }
  }
  ...
  getInputText() {
    ...
    return this.activeElement.isContentEditable ? this.activeElement.textContent : this.activeElement.value;
  }
